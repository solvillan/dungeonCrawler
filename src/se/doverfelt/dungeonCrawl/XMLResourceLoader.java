...
public class XMLResourceLoader {
    ...
    private float loadedResources;

    private float width = 500;

    private float getPercent(float whole, float fract, float width) {

        Log.debug("Percent: " + (fract/whole) + " Width in pixels: " + (fract/whole)*width);

        return (fract / whole) * width;

    }

    public void load(InputStream in, Graphics g, StateBasedGame sbg) throws IOException {
        try {
            ...
            Log.debug("Elements: " + elements[0].getAttributes().getLength());

            totalResources = elements[0].getElementsByTagName("sound").getLength() + elements[0].getElementsByTagName("music").getLength() + elements[0].getElementsByTagName("image").getLength() + elements[0].getElementsByTagName("sheet").getLength() + elements[0].getElementsByTagName("angelcodefont").getLength() + elements[0].getElementsByTagName("unicodefont").getLength() + elements[0].getElementsByTagName("spritesheetfont").getLength() + elements[0].getElementsByTagName("map").getLength() + elements[0].getElementsByTagName("param").getLength();
            loadedResources = 0;

            sbg.getContainer().getGraphics().setColor(Color.green);

            for (Element element : getElementsByTagName("sound")) {
                loadSound(element);
                loadedResources++;
                sbg.getContainer().getGraphics().fillRect(20, 20, 50, getPercent(totalResources, loadedResources, width));
                Start.loadScreen.render(sbg.getContainer(), sbg, sbg.getContainer().getGraphics());
                Start.loadScreen.update(sbg.getContainer(), sbg, 1);
            }
